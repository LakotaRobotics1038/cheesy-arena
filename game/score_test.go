...
func TestScoreSummary(t *testing.T) {
	...
	redSummary := redScore.Summarize("red", blueScore)
	...
	blueSummary := blueScore.Summarize("blue", redScore)
	assert.Equal(t, 16, blueSummary.FuelPoints)      // 7 auto + 9 teleop
	assert.Equal(t, 16, blueSummary.NumFuel)
	assert.Equal(t, 35, blueSummary.TowerPoints)     // Level1 Auto (15) + Level2 Teleop (20)
	assert.Equal(t, 51, blueSummary.MatchPoints)     // 16 + 35
	assert.Equal(t, 85, blueSummary.FoulPoints)      // 5 major (15pts each) + 2 minor (5pts each) = 75+10=85
	assert.Equal(t, 136, blueSummary.Score)          // 51 + 85
	assert.False(t, blueSummary.EnergizedRankingPoint) // 16 FUEL < 100
	assert.False(t, blueSummary.SuperchargedRankingPoint) // 16 FUEL < 360
	assert.False(t, blueSummary.TraversalRankingPoint) // 35 TOWER < 50
...
func TestScoreEnergizedRankingPoint(t *testing.T) {
	redScore := &Score{RedHub: Hub{AutoFuel: 100}}
	blueScore := &Score{BlueHub: Hub{}}

	summary := redScore.Summarize("red", blueScore)
	assert.True(t, summary.EnergizedRankingPoint)
	assert.Equal(t, 1, summary.BonusRankingPoints)

	// Below threshold
	redScore.RedHub.AutoFuel = 99
	summary = redScore.Summarize("red", blueScore)
...
func TestScoreSuperchargedRankingPoint(t *testing.T) {
	redScore := &Score{RedHub: Hub{AutoFuel: 200, TeleopFuel: 160}}
	blueScore := &Score{BlueHub: Hub{}}
	...
	// Below threshold
	redScore.RedHub.TeleopFuel = 159
	summary = redScore.Summarize("red", blueScore)
...
func TestScoreTraversalRankingPoint(t *testing.T) {
	...
	redScore := &Score{RedHub: Hub{AutoTowerLevel: TowerLevel1, TeleopTowerLevel: TowerLevel3}}
	blueScore := &Score{BlueHub: Hub{}}
